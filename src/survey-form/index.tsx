...
import {heads, tails} from "./questions.js";
import {ParticipantAnswers, uploadParticipantAnswers, UploadError} from "../utils/api.js";
import {
    Container,
...
import {REQUIRED_CORRECT_COUNT, STORAGE_KEYS, TOTAL_SETS, ATTENTION_CHECK_SETS} from "../utils/consts.js"
...
function SurveyForm({hideSurvey}: { hideSurvey: boolean }) {
    ...
    // Prolific, not implemented until testing starts
    ...
    }, [state.doRedirect, state.alertnessCorrectCount]);
    ...
    useEffect(() => {
        ...
        sessionStorage.setItem(STORAGE_KEYS.TABLE_ROWS, JSON.stringify(state.tableRows));
        sessionStorage.setItem(STORAGE_KEYS.CURR_SET, state.currSet.toString());
    ...
    React.useEffect(() => {
        ...
        if (saved) {
            ...
            if (Array.isArray(saved.attentionAnswers)) dispatch({
                type: 'SET_ATTENTION_ANSWERS',
                payload: saved.attentionAnswers
            });
            if (typeof saved.alertnessCorrectCount === 'number') dispatch({
                type: 'SET_ALERTNESS_COUNT',
                payload: saved.alertnessCorrectCount
            });
            if (typeof saved.IMCMistakeCount === 'number') dispatch({
                type: 'SET_IMC_MISTAKE_COUNT',
                payload: saved.IMCMistakeCount
            });
            if (Array.isArray(saved.tableRows)) dispatch({type: 'SET_TABLE_ROWS', payload: saved.tableRows});
            if (Array.isArray(saved.boldedVerbsInds)) dispatch({
                type: 'SET_BOLDED_VERBS_INDS',
                payload: saved.boldedVerbsInds
            });
            if (typeof saved.showAlertnessModal === 'boolean') dispatch({
                type: 'SET_SHOW_ALERTNESS_MODAL',
                payload: saved.showAlertnessModal
            });
